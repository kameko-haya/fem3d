...
!C===
      ...
      allocate (VAL_G(4*NNtotG), XYZ_G(3*NNtotG), WSarray(4*NNtotG))
...
!C-- NODE COORDINATE/VAL
     ...
     &     MPI_COMM_WORLD, ierr)
      ...
      do ip= 1, PETOT
        ...
        rcountsN(ip)= 4*rcountsN(ip) / 3
      enddo

      do i= 1, NP
        if (NODEflag(i).ne.0) then
          in= NODEflag(i)
          WSarray(4*in-3)= X(3*i-2)
          WSarray(4*in-2)= X(3*i-1)
          WSarray(4*in-1)= X(3*i  )
          WSarray(4*in  )= SIGMA_N(3*i)
        endif
      enddo

      call MPI_allGATHERv                                               &
     &    (WSarray, 4*NNlocal,            MPI_DOUBLE_PRECISION,         &
...
!C===
     ...
     &      ie, N1, ETYPE, in1, in2, in3, in4, in5, in6, in7, in8
        ...
        VAL_G_max= 0.d0
        do i= 1, NNtotG
          write (21,'(i8, 4(1pe16.6))')  i, (VAL_G(4*i-4+k), k=1,4)
          VAL_G_nod= VAL_G(4*i-3)**2 + VAL_G(4*i-2)**2 + VAL_G(4*i-1)
          if (VAL_G_nod.gt.VAL_G_max) then
            VAL_G_max= VAL_G_nod
            igmax    = i
          endif
        enddo
        ...
        write (*,'(i8, 3(1pe16.6),//)')  i, (VAL_G(4*i-4+k), k=1,3)
      endif
!C===
