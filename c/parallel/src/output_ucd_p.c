...
***/
	...
	VAL_G    =(KREAL *)allocate_vector(sizeof(KREAL),4*NNtotG);
	XYZ_G    =(KREAL *)allocate_vector(sizeof(KREAL),3*NNtotG);
	WSarray  =(KREAL *)allocate_vector(sizeof(KREAL),4*NNtotG);
...
***/
	...
	for(ip=1;ip<=PETOT;ip++){
		displsN [ip]= 4* displsN[ip]/3;
        	rcountsN[ip-1]= 4*rcountsN[ip-1]/3;
	}

	for(i=0;i<NP;i++){
		if( NODEflag[i] != 0 ) {
			in= NODEflag[i];
			WSarray[4*in-4]= X[3*i  ];
			WSarray[4*in-3]= X[3*i+1];
			WSarray[4*in-2]= X[3*i+2];
			WSarray[4*in-1]= SIGMA_N[3*i+2];
		}
	}

	MPI_Allgatherv( WSarray,  4*NNlocal, MPI_DOUBLE,
...
***/
	if (my_rank == 0 ){
		...
		for(i=0;i<NNtotG;i++){
			...
			i+1,VAL_G[4*i],VAL_G[4*i+1],VAL_G[4*i+2],VAL_G[4*i+3]);

			VAL_G_node=
			 VAL_G[4*i]*VAL_G[4*i]
			+VAL_G[4*i+1]*VAL_G[4*i+1]
			+VAL_G[4*i+2]*VAL_G[4*i+2];
			if( VAL_G_node > VAL_G_max ) {
				VAL_G_max=VAL_G_node;
				igmax=i+1;
			}
		}
		...
		fprintf(stdout,"%d %e %e %e\n",i+1,VAL_G[4*i],VAL_G[4*i+1],VAL_G[4*i+2]);
